...
| [`FINETUNE_CONTEXT.md`](FINETUNE_CONTEXT.md) / [`FINETUNE_PLAN.md`](FINETUNE_PLAN.md) / [`FINETUNE_COMPLETE.md`](FINETUNE_COMPLETE.md) | Our own fine-tuned answering model — why, the 5-step plan in easy English, and what shipped. Code lives in [`training/`](../training/). |
| [`FINETUNE_RESULTS.md`](FINETUNE_RESULTS.md) | The numbers: Claude vs `brainstack-3b` over the 22 golden questions, plus the investigation into the relevance gap. Regenerated by `training/compare_eval.py`. |
